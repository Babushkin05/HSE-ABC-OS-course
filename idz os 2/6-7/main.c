...
void run(size_t proger_num) {
    ...
    while (1) {
        ...
        if (task == -1) {  // new task
            ...
            sem_post(&buffer->task_sems[reviewer]);  // Notify reviewer
        } 
            ...
            if (is_correct) {
                ...
                sem_post(&buffer->task_sems[task]);  // Notify programmer
            } else {
                ...
                sem_post(&buffer->task_sems[proger_num]);  // Notify myself
            }
        } 
            ...
            if (is_correct) {
                ...
                sem_post(&buffer->task_sems[original_proger]);  // Notify programmer
            } else {
