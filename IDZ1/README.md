...
весь код и этот отчет в формате ```README.md``` можно найти по [ссылке](https://github.com/Babushkin05/HSE-ABC-OS-course/tree/main/IDZ1)
...
Однако я сделал исключения для макросов атомарных действий ввода/вывода. Оставил всю логику внутри их. Точно также они используются и в учебных материалах.
...
Требование не выполняется, потому что программа разработана с той идеей использования макросов которую описал выше. Сергей Александрович в чате разрешил так писать.
...
Действительно мои подпрограммы могут работать с разными подмассивами. Например тот же ```show_array``` в который передается адрес массива, его длины и название. Я вызываю подпрограмму два раза - для А и для В.
...
Добавил возможность автотестов, путем создания файла ```tester.asm```
...
и вот так это выглядит в терминале.
...
#### *Добавить в программу использование макросов которые должны использоваться в качестве оберток ранее написанных подпрограмм ввода и вывода данных, генерации тестовых массивов. Макросы должны поддерживать повторное использование с различными массивами и другими необходимыми параметрами.*

Что касается оберток писал уже выше в самом начале отчета. Макросы действительно поддерживают повторное использование с различными массивами и лругими параметрами.

#### *Допускается реализация в виде собственных макросов и других фрагментов программы.*

Допускается, но я этой возможностью не воспользовался.

### 10

#### *Программа должна быть разбита на несколько единиц компиляции (ассемблерных файлов). При этом подпрограммы ввода–вывода должны составлять унифицированные модули, используемые повторно как в программе, осуществляющей ввод или генерацию исходных данных, так и в программе, осуществляющей тестовое покрытие.*

Да программа разбита, даже в начале изобразил это разбитие. Подпрограммы ввода-вывода составляют унифицированные модули и используются повторно.

#### *Макросы должны быть выделены в отдельную автономную библиотеку*

Макросы ввода-вывода вынесены в отдельную библиотеку. А макросы для обработки массивов находятся в библиотеке со всей обработкой массивов. Такая структура мне показалась логичней.
