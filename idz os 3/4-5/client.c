...
int connect_with_retry(const char* ip, int port, int visitor_id) {
    ...
    for (int attempt = 0; attempt < MAX_RETRIES; attempt++) {
        ...
        if (inet_pton(AF_INET, ip, &serv_addr.sin_addr) <= 0) {
            printf("Visitor %d: Invalid address\n", visitor_id);
            close(sock);
            return -1;
        }
...
int send_and_wait_response(int sock, const char* message, char* response, int visitor_id) {
    ...
    while (attempts < MAX_ATTEMPTS) {
        ...
        if (bytes_read > 0) {
            ...
            printf("Visitor %d received: %s\n", visitor_id, response);
            return 0;
        } else if (bytes_read == 0) {
            printf("Visitor %d: Connection closed by server\n", visitor_id);
            return -1;
        } else {
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                continue;
            }
            printf("Visitor %d: Receave error: %s\n", visitor_id, strerror(errno));
            return -1;
        }
    }
...
void visit_gallery(int sock, int visitor_id) {
    ...
    if (send_and_wait_response(sock, buffer, response, visitor_id) != 0) return;
